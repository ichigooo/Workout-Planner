...
function validateWorkoutData(row, rowIndex) {
  ...
  if (!row.Category || !row.Category.trim()) {
    ...
    errors.push(`Row ${rowIndex + 1}: Invalid category "${row.Category}". Must be one of: ${VALID_CATEGORIES.join(', ')}`);
  }
  ...
  if (!isCardio) {
    ...
    if (!row.Reps || (row.Reps !== "—" && isNaN(parseInt(row.Reps)) && !row.Reps.includes("s") && !row.Reps.includes("min"))) {
      errors.push(`Row ${rowIndex + 1}: Reps must be a number, "—", or descriptive text for strength workouts`);
    }
  } else {
