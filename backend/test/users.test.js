...
describe('Users API', function() {
  ...
  it('POST /api/users should return 409 when creating user with duplicate email', async () => {
    ...
    const first = await request(BASE).post('/api/users').send(payload).set('Accept','application/json');
    expect([201,500]).to.include(first.status);

    // Second creation with same email should be 409 (or 500 if DB not available)
    const second = await request(BASE).post('/api/users').send(payload).set('Accept','application/json');
    if (first.status === 201) {
