    ...
    return (
        ...
        >
            <View style={styles.cardContent}>
                ...
                <Text style={[styles.dateRange, { color: theme.colors.subtext }]}>
                    {startDate} - {endDate}
                </Text>
                <Text style={[styles.workoutCount, { color: theme.colors.accent }]}>
    ...
    const loadPlans = async () => {
        try {
            ...
            const routine = ROUTINE_NAME
                ? (data || []).find(
                      (p) => (p.name || "").toLowerCase() === ROUTINE_NAME.toLowerCase(),
                  )
                : data && data.length > 0
                  ? data[0]
                  : undefined;
