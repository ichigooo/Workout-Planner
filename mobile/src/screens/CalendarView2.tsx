...
export default function CalendarView2() {
    ...
    const load = async () => {
      try {
        ...
        if (plans && plans.length > 0) {
          ...
          const now = new Date();
          const resp = await apiService.getPlanItemsByMonth(pid, now.getFullYear(), now.getMonth() + 1);
          if (!mounted) return;
  ...
  const getMarkedDates = () => {
    const marked: any = { [selectedDate]: { selected: true, selectedColor: theme.colors.accent } };
    monthItems.forEach(pi => {
      const sdRaw = (pi as any).scheduledDate ?? (pi as any).scheduled_date;
      if (!sdRaw) return;
      const datePart = (typeof sdRaw === 'string') ? sdRaw.split('T')[0].split(' ')[0] : (() => {
  ...
  const scheduledFromMonth: ScheduledWorkout[] = monthItems
    .map(pi => {
      ...
      return { id: `${pi.id}-${d}`, workout, date: d, intensity: pi.intensity || workout.intensity } as ScheduledWorkout;
    })
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <Text style={{ fontSize: 18, fontWeight: '700', color: theme.colors.text, marginBottom: 12 }}>Calendar</Text>
        <Calendar
        ...
        <View style={{ marginTop: 16 }}>
          <Text style={{ color: theme.colors.text, fontWeight: '700', marginBottom: 8 }}>{selectedDate === localTodayStr ? "Today's Workouts" : `${selectedDate} Workouts`}</Text>
          {selectedDayWorkouts.length > 0 ? (
            selectedDayWorkouts.map(s => (
              <View key={s.id} style={{ backgroundColor: theme.colors.surface, borderColor: theme.colors.border, borderWidth: 1, padding: 12, borderRadius: 12, marginBottom: 8 }}>
                <Text style={{ color: theme.colors.text, fontWeight: '600' }}>{s.workout.title}</Text>
                <Text style={{ color: theme.colors.subtext }}>{s.workout.sets} sets × {s.workout.reps} reps • {s.workout.category}</Text>
              </View>
            ))
          ) : (
